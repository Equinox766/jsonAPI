            ...
            if (request()->filled('sort'))
            {
                ...
                foreach ($sortFields as $sortField) {
                    ...
                    if (! in_array($sortField, $allowedSorts)) {
                        throw new BadRequestHttpException("No sort field '{$sortField}' is not allowed in the '{$this->getResourceType()}' resource.");
                    }
        ...
        return function ($allowedFilters) {
            ...
            foreach (request('filter', []) as $filter => $value) {
                ...
                if (! in_array($filter, $allowedFilters)) {
                    throw new BadRequestHttpException("No filter '{$filter}' is not allowed in the '{$this->getResourceType()}' resource.");
                }
        ...
        return function($allowedIncludes) {
            ...
            foreach ($includes as $include) {
                ...
                if (! in_array($include, $allowedIncludes)) {
                    throw new BadRequestHttpException("No include relationship '{$include}' is not allowed in the '{$this->getResourceType()}' resource.");
                }
                $this->with($include);
            }
